...
def evaluate_human_full_game(adversaries, num_trials):
    ...
    for i in range(num_trials):
        ...
        dead_scores_cont = [scores[-1] for i in range(conf.TIME_LIMIT - len(scores))]
...
cnn_food_model = DeepCNNModel(tau=TAU, gamma=GAMMA, eps_start=EPS_START, eps_end=EPS_END,
